...
export thresh, quello_principale, sample_neighbours, ab_thresh, act_thresh, ab_activity_reduction
...
function ab_thresh(mu, eps, α, m, ψ, w_u, γ, p_u)
    t_w_u = act_thresh(eps, α, w_u)
    t_p_u = act_thresh(eps, α, p_u)
    mean_a = (1 - α)*(1- eps^(2-α))/((2-α)* (1-eps^(1-α)))
    mean_a2 = (1 - α)*(1- eps^(3-α))/((3-α)* (1-eps^(1-α)))
    mean_a_w = (1 - α)*(t_w_u^(2-α)- eps^(2-α))/((2-α)* (1-eps^(1-α)))
    mean_a_p = (1 - α)*(t_p_u^(2-α)- eps^(2-α))/((2-α)* (1-eps^(1-α)))
    mean2_a_w = (1 - α)*(t_w_u^(3-α)- eps^(3-α))/((3-α)* (1-eps^(1-α)))
    mean2_a_p = (1 - α)*(t_p_u^(3-α)- eps^(3-α))/((3-α)* (1-eps^(1-α)))
    delta = 4* mean_a2 + (1-γ)*(mean_a_p^2 * (1-γ) - 4 * mean2_a_p)+
          (1-ψ)*(mean_a_w^2 * (1-ψ) - 4 * mean2_a_w)+
          2* (1-ψ)*(1-γ)*(2* min(mean2_a_p, mean2_a_w) - mean_a_p * mean_a_w)
    λ_t = 2 *mu* (m*(2*mean_a - (1-ψ)*mean_a_w - (1-γ)*mean_a_p + sqrt(delta)))^(-1)
    return λ_t, mean_a
end
...
function quello_principale(sim, eps, N_samp, α, i0, f, v, ψ, w_u, γ, p_u, r_s, r_i, α_m, mu, m, lambda)
...
df = CSV.read("/home/cmarinelli/Simulations/parameters_ab.csv", DataFrame, decimal = ',')
println(df)
tempo_inizio = now()
...
a = typeof(thresh)
println(a)
λ_t, media_a = ab_thresh(mu, eps, α, m, ψ, w_u, γ, p_u)
...
R0 = copy(lambda) ./ λ_t
println(R0)
RR = quello_principale(sim, eps, N_samp, α, i0, f, v, ψ, w_u, γ, p_u, r_s, r_i, α_m, mu, m, lambda)
