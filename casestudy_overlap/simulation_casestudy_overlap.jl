...
function overlap_states(N_samp, ψ, γ,α_m, adoption, overlap)
    ...
    if total_overlap + 2*total_excedence <= N_samp
        ...
        overlap_vec = copy(indices[1:total_overlap])
        exc1_vec = copy(indices[(total_overlap+1 ): (total_overlap+total_excedence)])
        exc2_vec = copy(indices[(total_excedence+total_overlap+1):(total_excedence*2+total_overlap)])
        ...
        vax_vec[copy(behav2)] .= 1
        S_red[copy(behav1)] .= ψ
        I_red[copy(behav1)] .= γ
    end
    return S_red, I_red, vax_vec
end
...
function quello_principale(sim, eps, N_samp, α, i0, f, v, ψ, w, γ, p, r_s, r_i, α_m, mu, m, lambda, adoption, overlap )
    ...
    #adoption = 0.2
    for n in 1:sim
        println("overlap = $overlap , adoption = $adoption")
