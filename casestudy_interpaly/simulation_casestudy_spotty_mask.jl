...
function overlap_states(N_samp, ψ, γ,α_m, adoption, overlap)
    ...
    if total_overlap + 2*total_excedence <= N_samp
        ...
        behav1 = append(overlap_vec, exc1_vec)
        behav2 = append(overlap_vec, exc2_vec)
        mask_red_i[behav2] .= α_m
        mask_red_s = copy(mask_i)
        S_red[behav1] .= ψ
        I_red[behav1] .= γ
    end
    return S_red, I_red, mask_red_s, mask_red_i
                    ...
                    if rand()<activities_s[i]
                        ...
                        for neighbour in neighbours
                            if infected_sim[neighbour]==1
                                if rand()<k*(((spotty_vec[i]>pro)+(spotty_vec[i]<pro)*(mask_s[i]))*((spotty_vec[neighbour]>pro)+(spotty_vec[neighbour]<pro)*(mask_i[neighbour])))
                                    append!(new_I, copy(i))
                                end
                            end
                        end
                    ...
                    if rand()<activities_i[i]
                        ...
                        for neighbour in neighbours
                            if susceptible_sim[neighbour] == 1
                                if rand()<k*(((spotty_vec[neighbour]>pro)+(spotty_vec[neighbour]<pro)*(mask_s[neighbour]))*((spotty_vec[i]>pro)+(spotty_vec[i]<pro)*(mask_i[i])))
...
function quello_principale(sim, eps, N_samp, α, i0, f, v, ψ, w, γ, p, r_s, r_i, α_m, mu, m, lambda)
    ...
    for n in 1:sim
        ...
        S_red, I_red , mask_s, mask_i = overlap_states(N_samp, ψ, γ,α_m, adoption, overlap)
        ...
        push!(R_vecs, copy(R_vec))
        println(typeof(R_vecs))
        push!(I_vecs, copy(I_vec))
    end
...
#hai fatto finpo a 6
df = CSV.read("/home/cmarinelli/Simulations/Parameters/parameters_casestudy_mask7.csv", DataFrame, decimal = ',')
println(df)
tempo_inizio = now()
...
println(lambda)
#println(R0)
R_vecs, I_vecs = quello_principale(sim, eps, N_samp, α, i0, f, v, ψ, w, γ, p, r_s, r_i, α_m, mu, m, lambda)
#println(R_vecs)
#println(typeof(R_vecs))
#RRdf = DataFrame(RR,Symbol.(R0))
a = ARGS[1]
filename_r = "datafile_r$a"
filename_i = "datafile_i$a"
...
CSV.write("/home/cmarinelli/Simulations/data_simulation/$filename_i.csv", dfi, decimal = '.')
CSV.write("/home/cmarinelli/Simulations/data_simulation/$filename_r.csv", dfr, decimal = '.')
