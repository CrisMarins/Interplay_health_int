...
function quello_principale(sim, eps, N_samp, α, i0, f, f_u, f_d, v, ψ, w, γ, p, r_s, r_i, α_m, mu, m, lambda)
...
#hai fatto finpo a 6
df = CSV.read("/home/cmarinelli/Simulations/Parameters/parameters_casestudy_vax7.csv", DataFrame, decimal = ',')
println(df)
tempo_inizio = now()
...
println(lambda)
#println(R0)
R_vecs, I_vecs = quello_principale(sim, eps, N_samp, α, i0, f, f_u, f_d, v, ψ, w, γ, p, r_s, r_i, α_m, mu, m, lambda)
#println(R_vecs)
#println(typeof(R_vecs))
